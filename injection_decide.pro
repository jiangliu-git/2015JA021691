function injection_decide, tplot_var, t0=t0_in, secsrange_ahead = secsrange_ahead, secsrange_after = secsrange_after, secs_injtype_pre = secs_injtype_aft, max = max, deh = deh, n_channels = n_channels, consecutive = consecutive, crit_inj = crit_inj, crit_noinj_all = crit_noinj_all, crit_noinj_channels = crit_noinj_channels, type = type, crit_dispersion = crit_dispersion, inj_energy_all = inj_energy_all, ratios = ratio_change, crit_nodeh = crit_nodeh, inf_good = inf_good
...
;; return value is 1 for ture, 0 for not, 2 for DEH, -1 for not sure, -2 for no data
;; max: if set, use max value for comparison; if not using avarage value.
;; deh: if set, the routine will exclude drift electron hole events for "no injection" group. Note that this routine itself cannot tell if the input is electron, 
;; crit_deh: criterion for is a DEH
;; crit_nodeh: criterion for not a DEH
...
if ~keyword_set(crit_deh) then crit_deh = 3. ;; the drop should be more than this to be a DEH
if ~keyword_set(crit_nodeh) then crit_nodeh = 2. ;; the drop should be less than this to be not a DEH
...
if keyword_set(secs_injtype_pre) then trange_injtype[0] = t0-secs_injtype_pre
if keyword_set(secs_injtype_aft) then trange_injtype[1] = t0+secs_injtype_aft
...
;; tell no injection
if keyword_set(deh) then begin
	is_deh = ratio_change lt 1./crit_nodeh
;	if strcmp(time_string(trange_ahead[0], precision = -3), '2013-06-20') then stop
endif else is_deh = 0
i_notnoinj_all = where(finite_req and ((ratio_change gt crit_noinj_all) or is_deh), n_notnoinj_all)
i_notnoinj_channels = where(finite_req and ((ratio_change gt crit_noinj_channels) or is_deh), n_notnoinj_channels)
	...
	if finite(total(max_ddt)) then begin
		...
		if (max(t_first_ex)-min(t_first_ex) lt crit_dispersion) and (n_last eq 0) and (n_first eq 0) then begin
			...
			if (product(diff_tddt le 0.) gt 0) and (n_last le 1) and (n_first le 1) then begin
				...
				if (product(diff_tddt ge 0.) gt 0) and (n_last le 1) and (n_first le 1) then begin
