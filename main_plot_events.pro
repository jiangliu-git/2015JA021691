...
;; diagnose
;events = events(*, [16,3]) ;; typical no injec and have injec for examples
events = events(*, [16,40]) ;; typical no injec and have injec for examples
...
size_x = 6.6
size_y = 10.7
abc_inj = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']
abc_noinj = ['j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r']
x_abc = replicate(0.19, n_elements(abc_inj))
y_abc = [0.934, 0.85, 0.755, 0.65, 0.53, 0.453, 0.365, 0.278, 0.183]
char_abc = 1.
...
;		if err eq 0 then begin
			rbsp_load_efw, trange=trange_load, probe=probes_load, rbsp_folder = rbsp_folder, /tclip
			...
			for j = 0, n_elements(probes_load)-1 do begin
				...
				if strcmp(purpose, 'publication') then begin
					ylim, 'th'+probes_load[j]+'_efw_esvy_gsm', -9., 19.
					ylim, 'th'+probes_load[j]+'_efw_vperp_gsm', -99., 99.
				endif
			endfor
;		endif else begin
...
;		catch, /cancel
	...
	for j = 0, n_elements(probes_load)-1 do begin
		...
		;; proton
		compute_moments, 'th'+probes_load[j]+'_pspec_combined_tclip', intype = 'flux', inunits_energy = 'keV', inunits_flux = 'keV', energy_min = energy_min, particle_types = 'p', rbsp_folder = rbsp_folder
		;; electron
	...
	options, 'th?_efw_vperp_gsm*', ytitle = 'V!dE'+cross+'B', ysubtitle = '!c[km]', labflag = 1
	;; filtered
	options, 'th?_fgs_gsm_tclip_hpfilt', ytitle = 'B filtered'
	options, 'th?_efw_esvy_gsm_hpfilt', ytitle = 'E filtered', ysubtitle = '!c[mV/m]'
	...
	if strcmp(plot_both, 'yes') then begin
		...
		if strcmp(plot_type, 'normal') then begin
			if strcmp(purpose, 'publication') then begin
				tvnames_plot = ['th'+sc+'_fgs_gsm_original', 'th'+sc+'_fgs_gsm_original_z', 'th'+sc+'_fgs_original_ttl', 'th'+sc+'_ion_density', 'th'+sc+'_all_Pall', 'th'+sc+'_mageis_pspec_official', 'th'+sc+'_mageis_espec_official', 'th'+sc+'_efw_esvy_gsm', 'th'+sc+'_efw_vperp_gsm']
				;tvnames_plot = ['th'+sc+'_fgs_gsm_original', 'th'+sc+'_fgs_gsm_original_z', 'th'+sc+'_fgs_original_ttl', 'th'+sc+'_ion_density', 'th'+sc+'_all_Pall', 'th'+sc+'_mageis_pspec', 'th'+sc+'_mageis_espec', 'th'+sc+'_efw_esvy_gsm', 'th'+sc+'_efw_vperp_gsm']
	...
	if strcmp(purpose, 'publication') then begin
		...
		;;; timebar for electric field and velocity
		timebar_mass, 0, /databar, varname = ['th'+sc+'_efw_esvy_gsm', 'th'+sc+'_efw_vperp_gsm'], line = 1
		;;; plot the time bar for the two DF ranges
		;; injection event
		timebar, ['2013-04-26/23:15:15', '2013-04-26/23:15:32'], varname = 'th'+sc+'_fgs_gsm_original_z';, line = 2
		;; no-injection event
		timebar, ['2013-06-02/00:06:01', '2013-06-02/00:06:30'], varname = 'th'+sc+'_fgs_gsm_original_z';, line = 2
	endif
